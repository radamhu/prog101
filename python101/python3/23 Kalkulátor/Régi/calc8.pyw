...
def torol(e=None):
    global eloz,akt
    if askyesno('Számológép','Törölsz mindent?'):
        eloz=['']
        akt=0
    kiir(eloz[akt])
...
def atlatszo(e=None):
    beallitas['alpha']=(1.0 if beallitas['alpha']==0.8 else 0.8)
    beallit()

def kilep(e=None):
    beallitas['hely'] = '+'+str(a.winfo_x())+'+'+str(a.winfo_y())
    with open('calc1.pck', 'wb') as bm:
        dump(beallitas, bm)
    with open('calc2.pck', 'wb') as em:
        dump(eloz, em)
    a.destroy()
