...
def elsofagy(ho):
    'Megadja az első fagyos nap indexét a ho listában'
    for i in range(len(ho)):
        if ho[i] <= 0:
            return i

def javit(ho,x):
    'Megnöveli x-szel ho minden elemét'
    for i in range(len(ho)):
        ho[i] += x
...
while True:
    ...
    if v == '1':
        ...
        print('Az első fagyos nap:',elsofagy(feb)+1)
    elif v == '5':
        k = int(input('Mennyit változtassak? '))
        javit(feb,k)
        print(feb)        
    elif v == '5':
        print('Javítás')
